...
def init_info():
    with open('./info.txt', 'a', 1, 'utf-8') as f:
...
class RadiomicsAnalysis:
    ...
    def svm_route(self):
        ...
        info_wt('svm_route')
        self.svm(model='svm')
        self.feature_weight_figure(model='svm')
        self.feature_correlation_figure(model='svm')
        self.lambda_selection_figure(model='svm')
        self.lambda_coefficient_figure(model='svm')
        self.roc_curve_figure(model='svm')

    def rf_route(self):
        init_info()
        info_wt('rf_route')
        self.random_forest(model='rf')
        self.feature_weight_figure(model='rf')
        self.feature_correlation_figure(model='rf')
        self.lambda_selection_figure(model='rf')
        self.lambda_coefficient_figure(model='rf')
        self.roc_curve_figure(model='rf')

    def pca_rf_route(self):
        init_info()
        info_wt('pca_rf_route')
        self.random_forest(model='pca_rf')
        self.feature_weight_figure(model='pca_rf')
        self.feature_correlation_figure(model='pca_rf')
        self.lambda_selection_figure(model='pca_rf')
        self.lambda_coefficient_figure(model='pca_rf')
        self.roc_curve_figure(model='pca_rf')

    def pca_svm_route(self):
        init_info()
        info_wt('pca_svm_route')
        self.svm(model='pca_svm')
        self.feature_weight_figure(model='pca_svm')
        self.feature_correlation_figure(model='pca_svm')
        self.lambda_selection_figure(model='pca_svm')
        self.lambda_coefficient_figure(model='pca_svm')
        self.roc_curve_figure(model='pca_svm')
    ...
    def data_preprocess(self, start_column_num=22):
        ...
        self.data = pd.read_excel('./nrrd_file/results_1680691841.xlsx').iloc[:, start_column_num:]
        ...
        for colName in self.data.columns[:]:
            if levene(self.data_train_a[colName], self.data_train_b[colName])[1] > 0.05:
                ...
                if ttest_ind(self.data_train_a[colName], self.data_train_b[colName], equal_var=False)[1] < 0.05:
                    index.append(colName)
        info_wt('t检验后, 筛选出', len(index), '个特征')
        info_wt('这些特征分别是:', index)
    ...
    def lasso(self):
        # LASSO特征筛选
        self.alphas = np.logspace(-3, 1, 50)  # alpha就是损失函数的拉姆达， 从10的-4次方到10的1次方均匀地取50个值
        ...
        info_wt('最优alpha值为:', self.model_lassoCV.alpha_)  # alpha代表选出来的最优alpha值
        info_wt('%s %d' % ('Lasso选择出的系数不为0的特征的个数:', sum(self.coef != 0)))
        info_wt('Lasso选择出的系数不为0的特征:', self.coef[self.coef != 0])
    ...
    def pca(self):
        ...
        info_wt(
            f'训练集中PCA降维前的数据形状为:{self.X_train_raw.shape}, PCA降维后的数据形状为:{self.X_train.shape}')
        info_wt('训练集中降维后每个特征的方差', model_pca.explained_variance_)
        info_wt('训练集中降维后每个特征能够解释原始信息的百分比', model_pca.explained_variance_ratio_)
        info_wt('训练集中降维后所有特征总共能解释原始信息的百分比', sum(model_pca.explained_variance_ratio_))

    def random_forest(self, model: str):
        # 随机森林分类器
        ...
        info_wt(f'{model}_训练集上的平均准确率为:', self.model.score(self.X_train, self.y_train))
        info_wt(f'{model}_测试集上的平均准确率为:', self.model.score(self.X_test, self.y_test))
        info_wt(f'{model}_测试集的正确结果为:', list(self.y_test))
        info_wt(f'{model}_对测试集进行结果预测的展示:', self.model.predict(self.X_test))
        info_wt(f'{model}_测试集中每种预测结果的概率展示:', self.model.predict_proba(self.X_test))
        info_wt(f'{model}_在拟合过程中用到了多少特征:', self.model.n_features_in_)
        info_wt(f'{model}_用到的每个特征的权重:', self.model.feature_importances_)
        info_wt(f'{model}_展示随机森林模型的各个参数:', self.model.get_params())

    def svm(self, model: str):
        # 支持向量机分类器
        ...
        info_wt(f'{model}_测试集中每种预测结果的概率展示:', self.model.predict_proba(self.X_test))
        info_wt(f'{model}_在拟合过程中用到了多少特征:', self.model.n_features_in_)
        info_wt(f'{model}_展示支持向量机模型的各个参数:', self.model.get_params())

    def feature_weight_figure(self, model):
        # 特征权重图
        ...
        plt.title(f'{model}_Weight of features', fontsize=22)
        # 创建结果图文件夹
        if not os.path.exists('./saved_images/'):
            os.makedirs('./saved_images/')
        plt.savefig(f'./saved_images/特征权重图_{model}.jpg', bbox_inches='tight', dpi=300)
        plt.show()

    def feature_correlation_figure(self, model: str):
        # 特征相关性热度图
        plt.figure(figsize=(12, 10), dpi=80)
        sns.heatmap(
        ...
        plt.title(f'{model}_Correlogram of features', fontsize=22)
        plt.xticks(fontsize=12)
        plt.yticks(fontsize=12)
        # 创建结果图文件夹
        if not os.path.exists('./saved_images/'):
            os.makedirs('./saved_images/')
        plt.savefig(f'./saved_images/特征相关性热度图_{model}.jpg', bbox_inches='tight', dpi=300)
        plt.show()

    def lambda_selection_figure(self, model: str):
        # LASSO模型中Lambda选值图
        ...
        plt.xlabel('Lambda')
        plt.ylabel('MSE')
        plt.title(f'{model}_Selection of lambda', fontsize=22)
        # 创建结果图文件夹
        if not os.path.exists('./saved_images/'):
            os.makedirs('./saved_images/')
        plt.savefig(f'./saved_images/LASSO模型中Lambda选值图_{model}.jpg', bbox_inches='tight', dpi=300)
        plt.show()

    def lambda_coefficient_figure(self, model: str):
        # 特征系数随Lambda变化曲线
        ...
        plt.title(f'{model}_Coefficient with lambda', fontsize=22)
        # 创建结果图文件夹
        if not os.path.exists('./saved_images/'):
            os.makedirs('./saved_images/')
        plt.savefig(f'./saved_images/特征系数随Lambda变化曲线_{model}.jpg', bbox_inches='tight', dpi=300)
        plt.show()

    def roc_curve_figure(self, model: str):
        ...
        # ROC曲线的AUC
        info_wt(f'{model}_ROC曲线的AUC为:', auc_score)
        #  精确度（Precision），敏感度（Sensitivity），特异度（Specificity）等输出
        #  将“1”类作为“阳性”时，“1”类的recall就是Sensitivity，“0”类的recall即为Specificity
        info_wt(f'{model}_分类报告如下:', classification_report(self.y_test, y_test_pred))

        plt.title(f'{model}_ROC Curve', fontsize=22)
        # 创建结果图文件夹
        if not os.path.exists('./saved_images/'):
            os.makedirs('./saved_images/')
        plt.savefig(f'./saved_images/ROC曲线_{model}.jpg', bbox_inches='tight', dpi=300)
        plt.show()
